...
struct SubAgentDelegationSection: IEnvironmentPromptSection {
    ...
    func append(to builder: inout String, context: EnvironmentPromptContext) {
        ...
        let toolName = settings.subAgent.toolName
        builder += "Use `\(toolName)` when a focused sub-run with tools and memory helps (research, multi-step file work, isolated experiments).\n"
